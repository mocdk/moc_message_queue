...
The queueworker is the process that asynchronously listens to messages ad dispatches these to listeners. The actual
...
started with debougOutput=TRUE)

::

	typo3/cli_dispatch.phpsh extbase queueworker:publishtestmessage TestMessage

And verify that the message TestMessage is printed to stdout where the queueworker is running.

All of the above will only work beanstalk is running on localhost (or the hos specified in the extension manager configuration)
...
the server name/ip-address, port and tube in the extension manager.
